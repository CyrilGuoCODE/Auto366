...
function createHarness(options = {}) {
    ...
    const sandbox = {
        ...
        Set,
        Map,
        state: { targetWrongCount: 0, uiMode: 'speaking', answerList: [], rebuildResults: [] },
        buildFillPlan(total, wrongCount) {
            const wrongNum = Math.max(0, Math.min(total, Number(wrongCount) || 0));
            return {
                wrongSet: new Set(Array.from({ length: wrongNum }, (_, i) => i)),
                correctCount: total - wrongNum,
                wrongCount: wrongNum,
            };
        },
        ...
        installHiddenChoices(count) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const options = ['Wrong A ' + i, wanted, 'Wrong C ' + i].map(body => {
                    ...
                    const radio = { click() { selected = true; selectedBody = body; } };
                    return {
                        innerText: body,
                        textContent: body,
                        querySelector(selector) {
                ...
                choiceSubs.push({
                    ...
                    get correctBody() { return wanted; },
                });
                // 高中答案结构：question 只是序号，真实题干在带 HTML 的 questionText。
                speaking.choices.push({
                    question: 'Question ' + (i + 1),
                    questionText: '<p style="background: transparent; text-align: justify">' + question + '</p>',
                    answer: 'B. ' + wanted,
                });
            }
        },
        setTargetWrongCount(count) {
            sandbox.state.targetWrongCount = count;
...
async function main() {
    ...
    const scoredChoices = createHarness();
    scoredChoices.installHiddenChoices(6);
    scoredChoices.setTargetWrongCount(2);
    assert.strictEqual(scoredChoices.speaking.fillChoices(false), 6, '控分模式仍须填完全部选择题');
    assert.strictEqual(
        scoredChoices.choiceSubs.filter(sub => sub.selectedBody !== sub.correctBody).length,
        2,
        '目标错 2 题时必须准确选择 2 个错误选项，而不是只改控制台文案',
    );
    console.log('PASS speaking score control: exactly two of six answers are intentionally wrong');
