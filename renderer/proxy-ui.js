class ProxyUI {
  ...
  // 停止代理
  async stopProxy() {
    const enhancementResult = await this.proxyEnhancement.onProxyStopping();
    if (!enhancementResult.success) {
      this.logManager.addErrorLog(`停止 TUN 代理增强失败: ${enhancementResult.message}`);
    }
  ...
  updateProxyStatus(data) {
    ...
    if (data.running) {
      ...
      this.proxyEnhancement.onProxyStarted().then((result) => {
        if (result.success && result.running && !result.alreadyRunning) {
          this.logManager.addSuccessLog('代理已启用 TUN 强制软包增强');
        } else if (!result.success && !result.cancelled) {
          this.logManager.addErrorLog(`TUN 代理增强启动失败: ${result.message}`);
        }
      });
    } else {
      this.state.isProxyRunning = false;
      this.proxyEnhancement.onProxyStopping();
